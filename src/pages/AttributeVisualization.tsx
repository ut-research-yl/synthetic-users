...
import {
  ...
  RadioButton, MessageStrip, Dialog, Bar, Menu, MenuItem, Switch, Link,
...
type Rule = { id: string; name: string; color: string; icon: string; showIcon: boolean; showValue: boolean; valueLabel: string; logic: 'and' | 'or'; conditions: Condition[] }
type Layer = {
  id: string; active: boolean; name: string; rules: Rule[]
}

const INITIAL_LAYERS: Layer[] = [
  { id: '1', active: true, name: 'KPI Definition', rules: [{ id: 'r1', name: '', color: '#009900', icon: 'pie-chart', showIcon: true, showValue: false, valueLabel: '', logic: 'and', conditions: [{ id: 'c1', property: 'KPI Definition', relation: 'is', value: 'Yes' }] }] },
  { id: '2', active: true, name: 'Requirements fulfilled', rules: [{ id: 'r2', name: '', color: '#99CC00', icon: 'accept', showIcon: true, showValue: false, valueLabel: '', logic: 'and', conditions: [{ id: 'c2', property: 'Status', relation: 'is', value: 'Build Phase' }] }] },
  { id: '3', active: true, name: 'Requirements open', rules: [{ id: 'r3', name: '', color: '#CCCC00', icon: 'circle-task', showIcon: true, showValue: false, valueLabel: '', logic: 'and', conditions: [{ id: 'c3', property: 'Status', relation: 'is', value: 'Design Phase' }] }] },
  { id: '4', active: true, name: 'Is critical component', rules: [{ id: 'r4', name: '', color: '#FF9933', icon: 'alert', showIcon: true, showValue: false, valueLabel: '', logic: 'and', conditions: [{ id: 'c4', property: 'Critical Application', relation: 'is', value: 'Yes' }] }] },
  { id: '5', active: true, name: 'SAP Transactions', rules: [{ id: 'r5', name: '', color: '#0066FF', icon: 'technical-object', showIcon: true, showValue: true, valueLabel: 'Yes', logic: 'and', conditions: [{ id: 'c5', property: 'SAP Executable', relation: 'is', value: 'Yes' }] }] },
]
...
export default function AttributeVisualization() {
  ...
  const [layerVis, setLayerVis] = useState<Record<string, Record<string, OverlayVis>>>(() =>
  ...
  const saveMatrix = () => { setIsDirty(true); setShowMatrix(false) }
  ...
  // Load icon path data for each rule in selected layer
  useEffect(() => {
  ...
  }, [selectedCfgLayerData?.id, selectedCfgLayerData?.rules.map(r => r.icon + r.showIcon).join()])
  ...
  const updateRule = (layerId: string, ruleId: string, updates: { color?: string; icon?: string; showIcon?: boolean; showValue?: boolean; valueLabel?: string; logic?: 'and' | 'or' }) => {
    setLayers(prev => prev.map(l => l.id !== layerId ? l : {
      ...l, rules: l.rules.map(r => {
        if (r.id !== ruleId) {
          // sync icon change to all rules
          return updates.icon !== undefined ? { ...r, icon: updates.icon } : r
        }
        return { ...r, ...updates }
      }),
    }))
  ...
  const addRule = (layerId: string) => {
    ...
    setLayers(prev => prev.map(l => l.id !== layerId ? l : {
      ...l, rules: [...l.rules, { id: ruleId, name: '', color: '#0066FF', icon: 'pie-chart', showIcon: true, showValue: false, valueLabel: '', logic: 'and' as const, conditions: [{ id: ruleId + '-c0', property: PROPERTIES[0], relation: 'is', value: '' }] }],
    }))
    setIsDirty(true)
  }
  ...
  const addLayerCfg = (name: string) => {
    ...
    setLayers(prev => [...prev, { id: newId, active: true, name, rules: [] }])
  ...
  return (
    ...
    <ObjectPage
      ...
      footerArea={isDirty ? (
        <Bar design="FloatingFooter">
          <Button slot="endContent" design="Emphasized" onClick={() => setIsDirty(false)}>Save</Button>
          <Button slot="endContent" onClick={() => { setLayers(INITIAL_LAYERS); setIsDirty(false) }}>Discard Changes</Button>
        </Bar>
      ) : undefined}
    >
      <ObjectPageSection id="modeler" titleText="Modeler" hideTitleText>
        ...
        <div style={{ display: 'flex', border: `1px solid ${border}`, borderRadius: 'var(--sapElement_BorderCornerRadius)', overflow: 'hidden', flex: 1, minHeight: 0, height: 'calc(100% - 1rem)', alignItems: 'stretch' }}>


          {/* ── Left: overlay list */}
          {true && (
            <div style={{ width: '20rem', flexShrink: 0, borderRight: `1px solid ${border}`, display: 'flex', flexDirection: 'column', background: 'white' }}>
              <div style={{ padding: '0.625rem 0.75rem 0.625rem 1rem', borderBottom: `1px solid ${border}`, background: 'var(--sapList_HeaderBackground)', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                ...
                <div>
                  <Button
                    ...
                    id="overlay-list-header-btn"
                    onClick={e => { e.stopPropagation(); setOpenMenu(openMenu === 'header' ? null : 'header') }}
                  />
                  <Menu
                    opener="overlay-list-header-btn"
                    open={openMenu === 'header'}
                    onClose={() => setOpenMenu(null)}
                    onItemClick={e => {
                      const text = (e.detail?.item as any)?.text
                      setOpenMenu(null)
                      if (text === 'Create Overlay') { setNewOverlayName(''); setNewOverlayDialogOpen(true) }
                      if (text === 'Visibility Settings') openMatrix()
                    }}
                  >
                  <MenuItem text="Create Overlay" />
                  <MenuItem text="Visibility Settings" />
                </Menu>
                </div>
              </div>
                    ...
                    >
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        <Text style={{ fontSize: 'var(--sapFontSmallSize)', color: 'var(--sapContent_LabelColor)', display: 'block' }}>
                          {layer.rules.length === 0 ? 'No rules' : `${layer.rules.length} rule${layer.rules.length !== 1 ? 's' : ''}`}
                        </Text>
                      </div>
                      <Button
                        id={`layer-menu-${layer.id}`}
                        icon="overflow"
                        design="Transparent"
          ...
          {selectedCfgLayerData ? (
              <div style={{ flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column', background: 'white' }}>
                <div style={{ padding: '8px 1rem 8px', background: 'var(--sapList_Background)', display: 'flex', alignItems: 'center' }}>                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                    <Title level="H5" style={{ fontStyle: selectedCfgLayerData.name ? 'normal' : 'italic' }}>
                      {selectedCfgLayerData.name || 'New overlay'}
                    </Title>
                    <Switch
                      checked={selectedCfgLayerData.active}
                      onChange={() => toggleActive(selectedCfgLayerData.id)}
                    />
                  </div>
                  {!selectedCfgLayerData.active && (
                    <Text style={{ fontSize: 'var(--sapFontSmallSize)', color: 'var(--sapContent_LabelColor)' }}>
                      This overlay is disabled and won't appear in the editor.
                    </Text>
                  )}
                </div>
                  ...
                  {selectedCfgLayerData.rules.length === 0 ? (
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.625rem', marginBottom: '0.75rem' }}>
                      {selectedCfgLayerData.rules.map((rule, ruleIdx) => {
                        ...
                        const colorBtnId = `color-btn-cfg-${selectedCfgLayerData.id}-${rule.id}`
                        const iconBtnId = `icon-btn-cfg-${selectedCfgLayerData.id}-${rule.id}`
                        ...
                        return (
                          <div key={rule.id} style={{ border: `1px solid ${border}`, borderRadius: 'var(--sapElement_BorderCornerRadius)', overflow: 'hidden' }}>
                            ...
                            {/* Row 1: Rule title + collapse + kebab */}
                            <div
                            ...
                            {/* Collapsible content */}
                            {!collapsedRules[rule.id] && <div>

                            {/* Style + Preview */}
                            <div style={{ display: 'flex' }}>
                              {/* Style controls */}
                              <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '0.75rem', padding: '16px 0.75rem 0 16px', background: 'var(--sapList_Background)' }}>
                                <Text style={{ fontSize: '14px', fontWeight: '600' }}>Style</Text>
                                <div style={{ display: 'flex', alignItems: 'flex-start', gap: '1.25rem', flexWrap: 'wrap' }}>

                                  {/* Icon */}
                                  <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem', opacity: (!rule.showIcon || ruleIdx > 0) ? 0.4 : 1 }}>
                                    <Label>Icon</Label>
                                    <SigChipV2
                                      id={iconBtnId}
                                      leadingIcon={rule.icon}
                                      value={ICON_OPTIONS.find(o => o.name === rule.icon)?.label ?? rule.icon}
                                      trailingIcon="slim-arrow-down"
                                      disabled={!rule.showIcon || ruleIdx > 0}
                                      tooltip={ruleIdx > 0 ? 'Icon is shared across all rules. Change it in the first rule.' : undefined}
                                      onClick={() => { if (rule.showIcon && ruleIdx === 0) { setIconPickerOpen(iconPickerOpen === iconBtnId ? null : iconBtnId); setIconSearch('') } }}
                                    />
                                    <Popover open={iconPickerOpen === iconBtnId} opener={iconBtnId} placement="Bottom" onClose={() => setIconPickerOpen(null)}>
                                        ...
                                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(8, 2rem)', gap: '0.125rem' }}>
                                          {ICON_OPTIONS.filter(o => !iconSearch.trim() || o.label.includes(iconSearch.toLowerCase()) || o.name.includes(iconSearch.toLowerCase())).map(o => (
                                            <Button key={o.name} icon={o.name} tooltip={o.label} design="Transparent"
                                              style={{ width: '2rem', height: '2rem', minWidth: 0, padding: 0, color: 'var(--sapTextColor)', border: rule.icon === o.name ? '2px solid var(--sapSelectedColor)' : 'none', borderRadius: '4px' } as React.CSSProperties}
                                              onClick={() => { updateRule(selectedCfgLayerData.id, rule.id, { icon: o.name }); setIconPickerOpen(null) }} />
                                          ))}
                                        </div>
                                      </div>
                                    </Popover>
                                      ...
                                      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                                        ...
                                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.35rem' }}>
                                          ...
                                          <div style={{ marginLeft: 'auto', display: 'flex', gap: '0.25rem' }}>
                                            <Button design="Transparent" disabled={!hexValid} onClick={() => { updateRule(selectedCfgLayerData.id, rule.id, { color: '#' + colorHex }); setOpenColorPicker(null) }}>OK</Button>
                                          </div>
                                          </>)})()}
                                        </div>
                                      </div>
                                  ...
                                  <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
                                    ...
                                    {(() => {
                                      const visMode = rule.showIcon && !rule.showValue ? 'icon' : rule.showIcon && rule.showValue ? 'both' : 'value'
                                      ...
                                      const visBtnId = `vis-btn-${rule.id}`
                                      return (
                                        <>
                                          <SigChipV2
                                            id={visBtnId}
                                          ...
                                          <Menu
                                            ...
                                            onItemClick={(e: any) => {
                                              const text = e.detail?.item?.text
                                              setOpenMenu(null)
                                              updateRule(selectedCfgLayerData.id, rule.id, {
                              ...
                              {/* Preview */}
                              <div aria-hidden="true" style={{ background: 'var(--sapGroup_ContentBackground, #f5f6f7)', border: `1px solid ${border}`, borderRadius: 'var(--sapElement_BorderCornerRadius)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem 1.25rem', flexShrink: 0, minWidth: '120px', margin: '0.75rem 16px 0 0' }}>
                                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start', gap: '3px' }}>
                                  {(rule.showIcon || rule.showValue) && (
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '3px', background: rule.color, borderRadius: '4px', width: rule.showIcon && !rule.showValue ? '20px' : 'auto', height: '20px', padding: rule.showValue ? '0 6px' : '0', justifyContent: 'center', flexShrink: 0 }}>
                                      {rule.showIcon && <Icon name={rule.icon} style={{ fontSize: '12px', width: '12px', height: '12px', color: ruleIconColor, pointerEvents: 'none' }} />}
                                      {rule.showValue && <Text style={{ fontSize: '10px', fontWeight: '600', color: ruleIconColor, fontFamily: "var(--sapFontFamily, '72', sans-serif)", whiteSpace: 'nowrap', lineHeight: '1' }}>{rule.valueLabel || 'Value'}</Text>}
                                    </div>
                                  )}
                            ...
                            {/* Conditions */}
                            <div style={{ padding: '0 0.75rem 0 16px', background: 'white' }}>
                              <Text style={{ fontSize: '14px', fontWeight: '600' }}>Conditions</Text>
                            </div>
                            <div style={{ display: 'flex', gap: '0.5rem', padding: '8px 0.75rem 0 16px', background: 'white' }}>
                            ...
                            {rule.conditions.map((cond, _condIdx) => {
                              ...
                              return (
                                <Fragment key={cond.id}>
                                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', padding: '0.25rem 0.75rem 0.5rem 16px', background: 'white', flexWrap: 'wrap' }}>
                                    <Select accessibleName="Condition property" style={{ flex: 2, minWidth: '10rem' }} onChange={e => updateCondition(selectedCfgLayerData.id, rule.id, cond.id, { property: (e.detail.selectedOption as HTMLElement).textContent ?? cond.property })}>
                                      {PROPERTIES.map(p => <Option key={p} selected={p === cond.property}>{p}</Option>)}
                                    </Select>
                            ...
                            <div style={{ padding: '0 0.5rem 16px 12px', background: 'var(--sapList_Background)' }}>
                ...
                <div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.375rem', marginBottom: '0.25rem' }}>
                    ...
                    <Link onClick={openMatrix} style={{ marginLeft: 'auto' } as React.CSSProperties}>Visibility Settings</Link>
                  </div>
                  <Text style={{ fontSize: 'var(--sapFontSize)', color: 'var(--sapContent_LabelColor)', display: 'block', marginBottom: '1rem' }}>
                  ...
                  <div>
                    ...
                    <Table
                      headerRow={
                        <TableHeaderRow>
                          <TableHeaderCell style={{ textAlign: 'center' }}>Audience</TableHeaderCell>
                          ...
                          <TableRow key={audience}>
                            ...
                            <TableCell>
                              <div style={{ display: 'flex', justifyContent: 'center' }}>
                                <RadioButton accessibleName={`${audience}: Enabled and Active`} name={`vis-${selectedCfgLayerData.id}-${audience}`} checked={currentVis === 'visible-active'} onChange={() => setLayerVisibility(selectedCfgLayerData.id, audience, 'visible-active')} />
                              </div>
                            </TableCell>
                            <TableCell>
                              <div style={{ display: 'flex', justifyContent: 'center' }}>
                                <RadioButton accessibleName={`${audience}: Enabled`} name={`vis-${selectedCfgLayerData.id}-${audience}`} checked={currentVis === 'visible'} onChange={() => setLayerVisibility(selectedCfgLayerData.id, audience, 'visible')} />
                              </div>
                            </TableCell>
                            <TableCell>
                              <div style={{ display: 'flex', justifyContent: 'center' }}>
                                <RadioButton accessibleName={`${audience}: Disabled`} name={`vis-${selectedCfgLayerData.id}-${audience}`} checked={currentVis === 'hidden'} onChange={() => setLayerVisibility(selectedCfgLayerData.id, audience, 'hidden')} />
                              </div>
                            </TableCell>
                          </TableRow>
                        )
      ...
      <ObjectPageSection id="pm-legacy" titleText="Process Manager (legacy)" hideTitleText>
        ...
        <div style={{ display: 'flex', border: `1px solid ${border}`, borderRadius: 'var(--sapElement_BorderCornerRadius)', overflow: 'hidden', flex: 1, minHeight: 0, height: 'calc(100% - 1rem)', alignItems: 'stretch' }}>
              ...
              <div style={{ padding: '0.625rem 0.75rem 0.625rem 1rem', borderBottom: `1px solid ${border}`, background: 'var(--sapList_HeaderBackground)', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                ...
                <div>
                  <Button
                    ...
                    icon="overflow"
                    style={{ opacity: 1, '--ui5-button-base-padding': '0 4px' } as React.CSSProperties}
                    id="overlay-list-header-btn-pm"
                    onClick={e => { e.stopPropagation(); setOpenMenu(openMenu === 'header-pm' ? null : 'header-pm') }}
                  />
                  <Menu
                    opener="overlay-list-header-btn-pm"
                    open={openMenu === 'header-pm'}
                    onClose={() => setOpenMenu(null)}
                    onItemClick={e => {
                      const text = (e.detail?.item as any)?.text
                      setOpenMenu(null)
                  ...
                  return (
                    <div
                      ...
                      style={{
                        ...
                        cursor: 'pointer',
                        display: 'flex', alignItems: 'center', gap: '0.5rem',
                        background: isSelected ? 'var(--sapList_SelectionBackgroundColor)' : 'white',
                        borderBottom: `1px solid ${border}`,
                        borderLeft: isSelected ? '3px solid var(--sapSelectedColor)' : '3px solid transparent',
                        opacity: layer.active ? 1 : 0.55,
                      }}
          ...
          {selectedCfgLayerData ? (
            <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'white' }}>
              <Text style={{ color: 'var(--sapContent_LabelColor)' }}>Select an overlay to configure it in the Modeler tab.</Text>
            </div>
          ) : (
            <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'white' }}>
              <Text style={{ color: 'var(--sapContent_LabelColor)' }}>Select an overlay from the list to configure it</Text>
    ...
    <Dialog
      open={showMatrix}
      headerText="Visibility Settings"
      style={{ width: 'fit-content', maxWidth: '90vw', maxHeight: '80vh' } as React.CSSProperties}
      onClose={cancelMatrix}
      footer={
        <Bar endContent={<><Button design="Emphasized" onClick={saveMatrix} disabled={!matrixHasChanges}>Save</Button><Button onClick={cancelMatrix} style={{ marginLeft: '0.5rem' } as React.CSSProperties}>Cancel</Button></>} style={{ '--ui5-bar-end-content-area-width': 'auto', paddingInlineEnd: '0' } as React.CSSProperties} />
      }
    >
                ...
                return (
                  <TableCell key={audience} style={{ padding: '0.4rem calc(0.5rem + 2.5px)' } as React.CSSProperties}>
                    <Select
                      ...
                      onChange={e => {
                        const text = (e.detail.selectedOption as HTMLElement).textContent?.trim() ?? ''
                        const v: OverlayVis = text === 'Enabled and Active' ? 'visible-active' : text === 'Enabled' ? 'visible' : 'hidden'
                        setLayerVisibility(layer.id, audience, v)
                      }}
                    >
                      <Option selected={vis === 'visible-active'}>Enabled and Active</Option>
                      <Option selected={vis === 'visible'}>Enabled</Option>
                      <Option selected={vis === 'hidden'}>Disabled</Option>
                    </Select>
                  </TableCell>
                )
              })}
    ...
    >
      <div style={{ padding: '1rem', display: 'flex', flexDirection: 'column', gap: '0.25rem', minWidth: '22rem' }}>
        <Label required for="new-overlay-name">Name</Label>
        <Input
          id="new-overlay-name"
    ...
    >
      <div style={{ padding: '1rem', display: 'flex', flexDirection: 'column', gap: '0.25rem', minWidth: '22rem' }}>
        <Label required for="rename-overlay-name">Name</Label>
        <Input
          id="rename-overlay-name"
          value={renameName}
          placeholder="Overlay name"
          style={{ width: '100%' }}
    ...
    >
      <div style={{ padding: '1rem', display: 'flex', flexDirection: 'column', gap: '0.25rem', minWidth: '22rem' }}>
        <Label required for="rename-rule-name">Name</Label>
        <Input
          id="rename-rule-name"
