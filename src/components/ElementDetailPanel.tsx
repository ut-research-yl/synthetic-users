...
import { Button, Icon, Input, Label, Switch, Tab, TabContainer, Text } from '@ui5/webcomponents-react'
...
const ELEMENT_ATTRS: Record<string, { label: string; value: string; type?: 'chip' | 'boolean' | 'select' }[]> = {
  'el-start': [
    { label: 'Event Type',       value: 'Start Event',   type: 'select' },
    { label: 'Trigger',          value: 'None',          type: 'select' },
    { label: 'Interrupting',     value: 'true',  type: 'boolean' },
    { label: 'Responsible Role', value: 'Recruitment Specialist' },
  ],
  'el-end1': [
    { label: 'Event Type',   value: 'End Event',    type: 'select' },
    { label: 'Result',       value: 'None',         type: 'select' },
    { label: 'Notification', value: 'Email',        type: 'select' },
    { label: 'Recipient',    value: 'Applicant'     },
  ],
  'el-end2': [
    { label: 'Event Type',   value: 'End Event',    type: 'select' },
    { label: 'Result',       value: 'None',         type: 'select' },
  ...
  'el-end4': [
    ...
    { label: 'Result',           value: 'None',                  type: 'select' },
    { label: 'Notification',     value: 'Email',                 type: 'select' },
    { label: 'Recipient',        value: 'New Hire'               },
    { label: 'Follow-up Action', value: 'Trigger Onboarding Flow' },
  ],
  'el-evaluate':  [
...
function LiCard({ ls, onSelect }: {
  ls: { id: string; widgetName: string; label?: string; shapeType: string }
  onSelect?: () => void
}) {
  const shapeIcon = LI_ICON[ls.shapeType] ?? 'SAP-icons-v4/data-indicator'
  return (
    <div onClick={onSelect} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '8px 0', cursor: 'pointer' }}>
...
export default function ElementDetailPanel({ elementId, onClose, linkedShapes, onSelectLinkedShape }: Props) {
  ...
  const [liExpanded, setLiExpanded] = useState(true)
  ...
  return (
    <SigRightSidePanel
      ...
      contentActionsSlot={[]}
      subHeaderSlot={linkedShapes && linkedShapes.length > 0 ? (
        <div style={{ padding: '0.375rem 0 0.5rem' }}>
          ...
          >
            ...
            <Text style={{ fontSize: 'var(--sapFontSmallSize)', color: 'var(--sapContent_LabelColor)', fontWeight: 700 } as React.CSSProperties}>
              Live Insights Data ({linkedShapes.length})
            </Text>
          </button>
          {liExpanded && (
            <div style={{ display: 'flex', flexDirection: 'column' }}>
              {linkedShapes.map(ls => (
                <LiCard key={ls.id} ls={ls} onSelect={() => onSelectLinkedShape?.(ls.id)} />
              ))}
            </div>
          )}
        </div>
